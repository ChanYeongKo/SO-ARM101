...
**핵심 목표**: 로봇이 쓰레기통을 한 번에 안정적으로 잡을 수 있도록 모션 파라미터를 튜닝

### 다음 단계

| 순서 | 작업 | 설명 |
|------|------|------|
| **현재** | 모션 시퀀스 최적화 | 팔 세우기→각도 세팅→하강→파지 흐름 튜닝 |
| 다음 1 | 다양한 위치 테스트 | 쓰레기통 위치를 바꿔가며 파지 성공률 측정 |
| 다음 2 | 물체 놓기(Place) | 파지 후 지정 위치에 놓는 동작 추가 |
| 다음 3 | 에러 복구 | IK 실패, 파지 실패 시 재시도 로직 |
| 다음 4 | 자동화 | 사람 개입 없이 검출→파지→놓기 전체 자동화 |
| 다음 5 | (선택) 학습 기반 | Classical 결과를 시드로 강화학습/모방학습 적용 |
